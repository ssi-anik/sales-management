...
public class Bill extends Database {
    ...
    public List<Map<String, String>> groupBy() {
        String query = "SELECT _id, bill_key, company_name, ( COUNT(price) || ' items selected' )  as items, ( 'Tk. ' || SUM(total_price)) as total, is_submitted as submission_status from " + this.table + " where is_ready = 1 GROUP BY bill_key ORDER BY _id DESC";
        db = getReadableDatabase();
        cursor = db.rawQuery(query, null);
        if (null == cursor || !cursor.moveToFirst()) {
            return null;
    ...
    public boolean insert(Map<String, String> data) {
        ...
        for (Map.Entry<String, String> column : data.entrySet()) {
            Log.v(AppConstant.TAG, "key: " + column.getKey() + " value: " + column.getValue());
