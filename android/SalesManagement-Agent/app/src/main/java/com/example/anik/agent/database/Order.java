...
public class Order extends Database {
    ...
    public List<Map<String, String>> groupBy() {
        String query = "SELECT _id, batch, company_name, ( COUNT(price) || ' items selected' )  as items, ( 'Tk. ' || SUM(total_price)) as total, is_submitted as submission_status from " + this.table + " GROUP BY batch ORDER BY _id DESC";
        db = getReadableDatabase();
        cursor = db.rawQuery(query, null);
        if (null == cursor || !cursor.moveToFirst()) {
            closeCursor();
